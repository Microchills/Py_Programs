...
def save_jobData(job_name,job_salary,job_week,job_time,job_details,
                job_location,com_name,com_desc):
    jobData = {
        'job_name': job_name,
        ...
        'job_details': [deta for deta in [re.sub('\\s', '',detail) 
...
def main():
    ...
    tasks = [asyncio.ensure_future(scrape_infoPage(url)) for url in urlList]
